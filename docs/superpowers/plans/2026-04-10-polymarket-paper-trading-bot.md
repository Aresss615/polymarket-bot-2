...
**Goal:** Build a paper trading bot that monitors Polymarket markets using two strategies — resolution arbitrage (matching news headlines to markets via Claude) and crypto level markets (betting on obvious price outcomes near expiry) — with a Rich terminal dashboard and CSV trade logging.

**Architecture:** Synchronous Python with threading. Engine loop polls Polymarket markets and news sources on intervals (30s tick, 60s news). Two strategies run independently each tick. Dashboard runs on the main thread via Rich Live. All trades are paper-only, logged to CSV.

**Tech Stack:** Python 3.14, requests, anthropic (claude-opus-4-6), feedparser, rich, python-dotenv, pytest
...
├── news_fetcher.py         # NewsAPI + Google News RSS
├── level_analyzer.py       # Crypto level strategy (regex + math)
├── arbitrage_analyzer.py   # Resolution arbitrage (Claude matching)
...
└── tests/
    ...
    ├── test_news_fetcher.py    # Mocked NewsAPI/RSS tests
    ├── test_level_analyzer.py  # Regex + signal logic tests
    ├── test_arbitrage_analyzer.py  # Mocked Claude tests
...
| `news_fetcher.py` | `fetch_newsapi() -> list[Article]`, `fetch_google_news() -> list[Article]`, `fetch_all_news() -> list[Article]` |
...
| `arbitrage_analyzer.py` | `analyze_headlines(articles, markets) -> list[Signal]` — calls Claude for reading comprehension matching |
...
## Task 6: news_fetcher.py — NewsAPI + Google News RSS
...
git commit -m "feat: add news fetcher with NewsAPI and Google News RSS"
```
...
## Task 8: arbitrage_analyzer.py — Resolution Arbitrage with Claude
...
git commit -m "feat: add resolution arbitrage analyzer with Claude matching"
```
...
Expected: All tests across all modules pass (approximately 53 tests).
...
|-----|----------|------|------------|
| Polymarket Gamma | `https://gamma-api.polymarket.com` | None | 15k/10s |
| Polymarket CLOB | `https://clob.polymarket.com` | None (read) | — |
| OKX v5 | `https://www.okx.com/api/v5` | None (public) | 20/2s |
| Bybit v5 | `https://api.bybit.com/v5` | None (public) | — |
| NewsAPI | `https://newsapi.org/v2` | API key | 100/day (free) |
| Google News RSS | `https://news.google.com/rss/search` | None | Unlimited |
